 ...
 *   masking server-side.
 */
export async function HorasScreen({
  searchParams,
  presentation,
}: HorasScreenProps) {
  ...
  // Hoje resolvido NO SERVIDOR e passado à grade: o cliente calcularia num fuso
  // possivelmente diferente do render do servidor, e a divergência cairia
  // justamente sobre o destaque do dia corrente.
  ...
  const header = (
    <PageHeader
      ...
      description={
        quiet
          ? "Lançamento semanal por projeto e atividade."
          : "Lançamento semanal por projeto e atividade, com envio para aprovação."
      }
      actions={
        <ViewSwitch
          ariaLabel="Tratamento visual da tela de Horas"
  ...
  if (!isDatabaseConfigured()) {
    return (
      <div className="space-y-6">
        {header}
        <TimesheetWeekView mode="demo" presentation={presentation} />
      </div>
    );
  }
