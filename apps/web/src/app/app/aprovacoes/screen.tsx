...
import type { SectionPanelVariant } from "@/components/ui/SectionPanel";
import {
  ApprovalQueue,
...
interface AprovacoesScreenProps {
  ...
  /** Tratamento visual servido por esta rota. */
...
/**
 * Corpo compartilhado da tela de Aprovações, servido pelas DUAS rotas:
 * `/app/aprovacoes` (Playful Ops) e `/app/aprovacoes/nova` (Operational
 * Minimal). Guard de papel, escopo por etapa, leitura e server actions são os
 * mesmos objetos nos dois casos — só `presentation` muda.
 *
 * É de propósito que isto seja UM componente e não duas telas: enquanto a
 * direção visual estiver em validação, qualquer divergência de comportamento
 * entre as rotas contaminaria a comparação.
 *
 ...
 */
export async function AprovacoesScreen({
  searchParams,
  presentation,
}: AprovacoesScreenProps) {
  // Keep in sync with the route-permissions map (/app/aprovacoes). O prefixo
  // cobre `/app/aprovacoes/nova`, então a rota nova herda o mesmo gate.
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={
          quiet
            ? "Triagem de horas e despesas pendentes."
            : "Triagem de horas e despesas pendentes com aprovação, reprovação justificada e histórico de decisões."
        }
        actions={
          <ViewSwitch
            ariaLabel="Tratamento visual da tela de Aprovações"
            current={quiet ? QUIET_HREF : CLASSIC_HREF}
            options={[
              { label: "Clássica", href: CLASSIC_HREF },
              { label: "Nova", href: QUIET_HREF },
            ]}
          />
        }
      />
